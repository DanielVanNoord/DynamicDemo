...
namespace ASCOM.DynamicDemo.SafetyMonitor
{
    //
    ...
    //
    // Your driver's DeviceID is ASCOM.DynamicDemo.SafetyMonitor
    //
    // The COM Guid attribute sets the CLSID for ASCOM.DynamicDemo.SafetyMonitor
    // The COM ClassInterface/None attribute prevents an empty interface called _DynamicDemo from being created and used as the [default] interface
    //
    ...
    public class SafetyMonitor : ReferenceCountedObjectBase, ISafetyMonitor, IDisposable
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="DynamicDemo"/> class. Must be public to successfully register for COM.
        /// </summary>
        public SafetyMonitor()
        {
            try
            {
                ...
                tl = new TraceLogger("", "DynamicDemo.Driver"); // Remove the leading ASCOM. from the ProgId because this will be added back by TraceLogger.
                SetTraceState();
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.DynamicDemo.SafetyMonitor", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
